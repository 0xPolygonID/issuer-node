...
// A function that accepts 2 generic types to generate a function that accepts a parser if the types match, otherwise it raises a static error).
...
>(
  ...
  > extends true
    ? // 2. then if the output types match exactly
      ...
      ? // 3. then if the input types match exactly,
        ...
        ? // 3. return type as ZodSchema (it is certain that I/O and parser I/O are identical)
          ZodSchema<ParserOutput, ZodTypeDef, ParserInput>
        : // 3. fail
          never
      : // 2. fail
  ...
  return <Output, Input = Output>(parser: unknown) =>
