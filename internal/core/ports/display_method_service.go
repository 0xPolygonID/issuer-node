...
type DisplayMethodService interface {
	Save(ctx context.Context, issuerDID w3c.DID, name, url string) (*uuid.UUID, error)
	Update(ctx context.Context, identityDID w3c.DID, id uuid.UUID, name, url *string) (*uuid.UUID, error)
