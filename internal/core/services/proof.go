...
import (
	...
	"github.com/polygonid/issuer-node/pkg/credentials/signature/circuit/signer"
...
func (p *Proof) findClaimForQuery(ctx context.Context, identifier *w3c.DID, query ports.Query) ([]*domain.Claim, error) {
	...
	claim, _, err := p.claimsRepository.GetAllByIssuerID(ctx, p.storage.Pgx, *identifier, filter)
