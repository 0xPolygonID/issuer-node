...
func TestServer_GetStateStatus(t *testing.T) {
	...
	idenWithSignatureClaim, err := serverWithSignatureClaim.Services.identity.Create(ctx, "polygon-test", &ports.DIDCreationOptions{Method: method, Blockchain: blockchain, Network: network, KeyType: BJJ})
	...
	idenWithMTPClaim, err := serverWithMTPClaim.Services.identity.Create(ctx, "polygon-test", &ports.DIDCreationOptions{Method: method, Blockchain: blockchain, Network: network, KeyType: BJJ})
	require.NoError(t, err)
