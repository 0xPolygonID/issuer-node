	...
	for _, tc := range []testConfig{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			case GetCredential200JSONResponse:
				...
				assert.NoError(t, json.Unmarshal(rr.Body.Bytes(), &response))
				validateCredential(t, response, GetClaimResponse(v))
...
func TestServer_GetCredentials(t *testing.T) {
	...
	for _, tc := range []testConfig{
		...
		t.Run(tc.name, func(t *testing.T) {
			rr := httptest.NewRecorder()
			tURL := getCredentialsURL(tc.did, tc.filter.schemaHash, tc.filter.schemaType, tc.filter.subject, tc.filter.revoked, tc.filter.self, tc.filter.queryField)
...
func validateCredential(t *testing.T, resp, tc GetClaimResponse) {
	t.Helper()
...
func getCredentialsURL(did string, schemaHash *string, schemaType *string, subject *string, revoked *string, self *string, queryField *string) string {
